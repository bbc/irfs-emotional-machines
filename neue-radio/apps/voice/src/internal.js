      ...
      if (type === 'full') {
        rasa.getIntent(transcript, 'emotional_robots').then((intent) => {
          ...
          if (intent === 'bad_robot') {
            ...
            websocket.publish({ topic: `robot/command/turn-away` });
          ...
          } else if (intent == 'radio_one_please'){
            websocket.publish({ topic: `speech/command/speak`, payload: { utterance: 'i am not a fan of bbc radio one but since you asked politely' } });
            websocket.publish({ topic: `robot/command/radio-one-please` });
